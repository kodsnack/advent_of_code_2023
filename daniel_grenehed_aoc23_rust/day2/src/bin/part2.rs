        ...
        match create_game(line.to_string()) {
            Some(game) => {
                ...
                for set in game.sets {
                    match set.color {
                        Color::Red => { r = if set.count > r { set.count } else { r } },
                        Color::Green => { g = if set.count > g { set.count } else { g } },
                        Color::Blue => { b = if set.count > b { set.count } else { b } }
                    }
                }
                sum += r*g*b;
            },
            None => {
                println!("Failed to create game from '{}'", line);
            }
        }
    }
    sum 
